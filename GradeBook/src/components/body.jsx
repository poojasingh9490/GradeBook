...
function Body() {
  ...
  const filterStudents = (status) => {
    const filteredData = students.filter((student) => student.status === status);
  ...
  return (
    <div id='styl'>
      <div id='lst'>
      <button className="btn" onClick={() => sortStudents("marks")}>
          <FontAwesomeIcon icon="filter" size="lg" /> Sort by Grade {sortType === "marks" && (sortOrder === "asc" ? "▲" : "▼")}
        </button>

        <button className="btn" onClick={() => sortStudents("name")}>
          <FontAwesomeIcon icon="sort-alpha" size="lg" /> Sort A-Z {sortType === "name" && (sortOrder === "asc" ? "▲" : "▼")}
        </button>

        <button className="btn" onClick={() => sortStudents("name")}>
          <FontAwesomeIcon icon="sort-alpha" size="lg" /> Sort Z-A {sortType === "name" && (sortOrder === "desc" ? "▲" : "▼")}
        </button>

        <button className="btn" onClick={() => filterStudents("pass")}>
          Pass
        </button>

        <button className="btn" onClick={() => filterStudents("fail")}>
          Fail
        </button>

        <input class = "search" type="text" placeholder="Enter the Name" value={searchTerm} onChange={searchStudents} />
        {noResultsFound && <h2>Sorry! No Results Found.....</h2>}
      </div>
      <table>
        <thead>
          <tr>
            <th><FontAwesomeIcon icon="hashtag" size="sm" /> No</th>
            <th>Name</th>
            <th>Ticket</th>
            <th>Rating Grade</th>
            <th>Exam Grade</th>
            <th>Overall Grade</th>
            <th>Status</th>
            <th><FontAwesomeIcon icon="circle-info" size="sm" /> Details</th>
          </tr>
        </thead>
        <tbody>
          {sortedStudents
            .slice(0, showAllRows ? sortedStudents.length : 10)
            .map((student, index) => (
              <tr
                key={index}
                onClick={(event) => {
